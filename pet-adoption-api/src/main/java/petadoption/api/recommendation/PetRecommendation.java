...
public class PetRecommendation {
    ...
    // computes cosine similarity between two vectors (a user's preferences and a
    // pet's attributes)
    public static double calcPetSimilarity(User u, Pet p) {
         ...
         * It finds the angle between 2 vectors (which are the user/pet attribute
         * arrays).
         * 
         * We want the magnitude of some values to matter because of the nature of the
         * attributes, such as age. We want to
         * consider the magnitude of the age because it is a linear value. But we don't
         * want to consider the magnitude of
         * a value such as species, because it isn't a linear value. We don't consider
         * the distance between a cat and a dog
         * as any different from the distance between a cat and a rabbit.
         * 
         * The way that I have some values' magnitudes mattering and some not, is using
         * one-hot encoding. This basically
         * means that for values where magnitude doesn't matter, we have them stored in
         * multiple spaces in the array.
         * For example, species is stored in indices 0,1, and 2. If a pet is a cat,
         * species is stored as [1][0][0], but a
         * dog would be stored as [0][1][0], and a rabbit would be [0][0][1]. This
         * allows for species to still make a
         * difference in the return value, but have no difference in distance between
         * different species.
         * For values where magnitude matters, we leave them as is, where their actual
         * value is stored.
         * 
         */
        ...
        if (dotProduct == 0.0) {
            return 0.0;
        } // if user preferences are empty, return 0
